            ...
            for font_dir in paths_to_check:
                if os.path.exists(font_dir):
                    for file in os.listdir(font_dir):
                        if (
                            file.lower() == font_name.lower()
                            or os.path.splitext(file)[0].lower() == font_name.lower()
                        ):
                            font_path = os.path.join(font_dir, file)
                            try:
                                # Windows需要先取消注册
                                if self.system == "windows":
                                    self._windows_unregister_font(
                                        font_path, font_dir in self.font_paths["user"]
                                    )
...
if __name__ == "__main__":
    ...
    for font in fonts:
        ...
        print(
            f"    族名: {detailed_info.family_name}, 样式: {detailed_info.style_name}"
        )
