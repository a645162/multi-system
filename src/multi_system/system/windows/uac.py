    ...
    def run_as_admin(
        ...
        try:
            if self.is_admin():
                ...
                # 需要UAC提权
                return self._run_elevated_command(
                    command, wait, capture_output, working_dir
                )
    ...
    def _run_elevated_command(
        ...
        try:
            if capture_output:
                ...
                result = subprocess.run(
                    ["powershell", "-Command", ps_command], capture_output=False
                )
...
"""
            ...
            # 以管理员身份执行批处理脚本
            ps_command = (
                f'Start-Process "{script_file}" -Verb RunAs -Wait -WindowStyle Hidden'
            )
    ...
    def run_powershell_as_admin(
        ...
        try:
            if self.is_admin():
                ...
                if wait:
                    return subprocess.CompletedProcess(
                        args=script, returncode=result.returncode
                    )
                return None
    ...
    def check_uac_level(self) -> dict:
        ...
        try:
            ...
            with winreg.OpenKey(
                ...
                try:
                    uac_info["ConsentPromptBehaviorAdmin"] = winreg.QueryValueEx(
                ...
                try:
                    uac_info["ConsentPromptBehaviorUser"] = winreg.QueryValueEx(
                ...
                try:
                    uac_info["PromptOnSecureDesktop"] = winreg.QueryValueEx(
            ...
            if uac_info["EnableLUA"] == 0:
                ...
                uac_info["ConsentPromptBehaviorAdmin"] == 5
                and uac_info["PromptOnSecureDesktop"] == 0
            ):
                uac_info["level"] = "仅在应用尝试更改时通知（不调暗桌面）"
                uac_info["description"] = "中等安全级别"
            elif (
                uac_info["ConsentPromptBehaviorAdmin"] == 5
                and uac_info["PromptOnSecureDesktop"] == 1
            ):
...
if __name__ == "__main__":
    ...
    # PowerShell命令测试
    ps_result = uac.run_powershell_as_admin(
        "Get-Process | Select-Object -First 5", capture_output=True
    )
    if ps_result:
